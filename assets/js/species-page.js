...
$(document).ready(() => {
      ...
      if (!problems.includes(prop))
        ...
        switch (prop) {
          ...
          case "hardiness":
            ...
            const liText =
              min === max
                ? `${formattedProp}: Zone ${min}`
                : `${formattedProp}: Zone ${min} to Zone ${max}`;
            newLi.text(liText);
            break;
          case "watering_general_benchmark":
            newLi.text(
              `${formattedProp}: ${data[prop].value} ${data[prop].unit}`
            );
            break;
          case "sunlight":
          case "propagation":
  ...
  fetch(
    `https://perenual.com/api/species/details/${plantId}?key=${plantApiKey}`,
    requestOptions
  )
    .then((res) => {
